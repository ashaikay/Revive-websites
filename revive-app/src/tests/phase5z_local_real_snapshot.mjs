...
async function identity(label) {
  const email = 'phase5z-'  + Date.now() + '-' + label + '@example.test';
...
const workspace = await rpc(owner.token, 'create_workspace_with_owner', { workspace_name: 'Phase 5Z ' + stamp, workspace_slug: 'phase-5z-' + stamp });
const other = await rpc(outsider.token, 'create_workspace_with_owner', { workspace_name: 'Phase 5Z Other ' + stamp, workspace_slug: 'phase-5z-other-' + stamp });
...
console.log('PHASE5Z_REAL_SNAPSHOT=' + (failures ? 'FAIL' : 'PASS'));
