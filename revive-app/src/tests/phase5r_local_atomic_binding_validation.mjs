...
const workspace = await rpc(owner.token, 'create_workspace_with_owner', { workspace_name: 'Phase 5R ' + stamp, workspace_slug: 'phase-5r-' + stamp });
const other = await rpc(outsider.token, 'create_workspace_with_owner', { workspace_name: 'Phase 5R Other ' + stamp, workspace_slug: 'phase-5r-other-' + stamp });
...
const replay = await rpc(admin.token, 'reserve_rev_meeting_event_execution_bound', { ...args, target_request_id: randomUUID() });
...
check('WRONG_EXPECTED_VERSION_DENIED', (await rpc(owner.token, 'reserve_rev_meeting_event_execution_bound', { ...args, expected_binding_version: 2 })).status >= 400);
check('WRONG_EXPECTED_TIMEZONE_DENIED', (await rpc(owner.token, 'reserve_rev_meeting_event_execution_bound', { ...args, expected_timezone: 'UTC' })).status >= 400);
...
const concurrentCalls = await Promise.all(Array.from({ length: 12 }, (_, i) =>
  rpc(i % 2 ? admin.token : owner.token, 'reserve_rev_meeting_event_execution_bound', {
    target_request_id: randomUUID(), target_workspace_id: workspaceId, target_action_id: concurrentActionId,
    expected_calendar_reference: args.expected_calendar_reference, expected_timezone: args.expected_timezone, expected_binding_version: args.expected_binding_version,
  })));
...
check('CHANGED_CALENDAR_CONFLICTS_WITH_REPLAY', (await rpc(owner.token, 'reserve_rev_meeting_event_execution_bound', args)).status >= 400);
...
check('REPLAY_RECHECKS_BINDING', (await rpc(owner.token, 'reserve_rev_meeting_event_execution_bound', args)).status >= 400);
...
console.log('PHASE5R_LOCAL_ATOMIC_BINDING=' + (failures ? 'FAIL' : 'PASS'));
