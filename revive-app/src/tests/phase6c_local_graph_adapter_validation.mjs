...
import { createMicrosoftGraphCalendarEvent } from '../../supabase/functions/_shared/microsoftGraphCalendarEvent.ts';
...
async function identity(label) {
  const email = 'phase6c-'  + Date.now() + '-' + label + '@example.test';
...
const workspace = await rpc(owner.token, 'create_workspace_with_owner', { workspace_name: 'Phase 6C ' + stamp, workspace_slug: 'phase-6c-' + stamp });
const other = await rpc(outsider.token, 'create_workspace_with_owner', { workspace_name: 'Phase 6C Other ' + stamp, workspace_slug: 'phase-6c-other-' + stamp });
...
console.log('PHASE6C_LOCAL_GRAPH_ADAPTER=' + (failures ? 'FAIL' : 'PASS'));
